...
const faqs = [
  ...
  { q: 'Fixlyy peut-il gérer plusieurs numéros ?', a: "Oui, dès le plan Entreprise. Jusqu'à 3 numéros pour plusieurs techniciens." },
  { q: 'Que se passe-t-il si je dépasse 150 appels (Essentiel) ?', a: '0,50€/appel supplémentaire. Ou passez au plan Avancé (illimité à 149€/mois).' },
];
      ...
      {faqs.map((item, i) => (
        <div key={i}
          className={`bg-white border rounded-2xl overflow-hidden transition-all duration-200 ${
            open === i ? 'border-[#DDD6FE] shadow-[0_4px_16px_rgba(124,58,237,0.08)]' : 'border-[#E5E7EB]'
          }`}>
          <button
            onClick={() => setOpen(open === i ? null : i)}
            className="w-full flex items-center justify-between px-5 sm:px-6 py-4.5 text-left hover:bg-[#F5F3FF] transition-colors group">
            <span className={`font-semibold pr-4 text-sm sm:text-base transition-colors ${open === i ? 'text-[#7C3AED]' : 'text-[#111827]'}`}>
              {item.q}
            </span>
            <span className={`flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center transition-all ${
              open === i ? 'bg-[#7C3AED] text-white' : 'bg-[#F5F3FF] text-[#7C3AED] group-hover:bg-[#EDE9FE]'
            }`}>
